...
from sealy import (BatchDecryptor, BatchEncryptor, CiphertextBatchArray,
                   CKKSBatchEncoder, Context, KeyGenerator)

from fhelwr.model import (ciphertext_to_params, flatten_parameters,
                          params_to_ciphertext, unflatten_parameters)


class SealyScheme(ABC):
    """
    The encryption schemes supported by SEALY.
    """
    ...

class CKKS(SealyScheme):
    """
...
class SealyClient(fl.client.Client, ABC):
    ...
    def __init__(self, cid: int, context: Context, scheme: SealyScheme) -> None:
        """
    ...
    def __encrypt_params(self, parameters: np.ndarray) -> CiphertextBatchArray:
        ...
        """
        encoded = self.__get_encoder().encode(parameters.tolist())
        encrypted = self.__get_encryptor().encrypt(encoded)
        return encrypted

    def __decrypt_params(self, encrypted: CiphertextBatchArray) -> np.ndarray:
        decrypted = self.__get_decryptor().decrypt(encrypted)
        decoded = np.asarray(self.__get_encoder().decode(decrypted))
        return decoded

    def __get_encoder(self) -> CKKSBatchEncoder:
        """
        Return the encoder used by this client.
        """
        if isinstance(self.scheme, CKKS):
            return CKKSBatchEncoder(self.ctx, self.scheme.scale) 
        else:
            raise ValueError(f"Unsupported scheme: {self.scheme}")

    def __get_encryptor(self) -> BatchEncryptor:
        """
        Return the encryptor used by this client.
        """
        return BatchEncryptor(self.ctx, self.keygen.create_public_key())

    def __get_decryptor(self) -> BatchDecryptor:
        """
        Return the decryptor used by this client.
        """
        return BatchDecryptor(self.ctx, self.keygen.secret_key())
    ...
    def get_parameters(self, ins: GetParametersIns) -> GetParametersRes:
        ...
        # Serialize to bytes
        logging.debug("Serializing encrypted parameters for client %d", self.cid)
        parameters = ciphertext_to_params(encrypted_parameters)
        logging.debug("Serialized encrypted parameters for client %d", self.cid)
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        parameters = params_to_ciphertext(self.ctx, ciphered_parameters)
        # Decrypt parameters
        ...
        return FitRes(parameters=parameters, num_examples=num_examples, status=status, metrics={})
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        # Status
        status = Status(code=Code.OK, message="Success")
        return EvaluateRes(loss=loss, num_examples=num_examples, status=status, metrics=metrics)
